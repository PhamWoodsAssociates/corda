...
class ConnectionTracker(val serverMode: Boolean,
                        collector: Collector,
                        val localLegalName: String,
                        val remoteLegalName: String) : BaseHandler() {
    companion object {
        private val log = loggerFor<ConnectionTracker>()
    ...
    init {
        ...
        if (serverMode) {
            ...
            sasl.plain("SystemUsers/Peer", "SystemUsers/Peer")
            sasl.server()
    ...
    override fun onDelivery(event: Event) {
        ...
        if (link is Receiver) {
            if (delivery.isReadable && !delivery.isPartial) {
                log.info("Full message received")
                ...
                val channel = connection?.context as? Channel
                if (channel != null) {
                    val appProperties = HashMap<Any?, Any?>(amqpMessage.applicationProperties.value)
            ...
            log.info("Sender delivery confirmed ${String(delivery.tag)}")
    ...
    fun transportWriteMessage(msg: SendableMessageImpl) {
        log.info("Queue application message write ${javax.xml.bind.DatatypeConverter.printHexBinary(msg.payload)}")
